...
class MCTS:
    ...
    def monte_carlo_tree_search(self, node):
        ...
        for i in range(computation_budget):
            expend_node = self.tree_policy(node)
            reward = self.default_policy(expend_node)
            self.backup(expend_node, reward)
...
def IsOverlap(vehilce_type, state):
    is_over_lap = False
    if(vehilce_type == "ego"):
        if((state.y > -80 and state.y < -20 and state.x > 0 and state.x < 10) or
           (state.y > 20 and state.y < 80 and state.x > 0 and state.x < 10) or
           (state.y > -20 and state.y < 20 and state.x < 10 and state.x > 0)):
            ...
            is_over_lap = True
    if(vehilce_type == "car"):
        if((state.y > 20 and state.y < 80 and state.x > -10 and state.x < 0) or
           (state.y > -20 and state.y < 20 and state.x > -10) or
           (state.x > 10 and state.y > -20 and state.y < 0)):
            if(math.sin(state.theta) >= 0):
                is_over_lap = True
            else:
                is_over_lap = False
...
def Clothoid2DiscretizingPts(s_x, s_y, s_theta, s_curvature, e_x, e_y, e_theta, e_curvature) -> dict:
    discretize_pts = dict()
    total_length = 0.0
    x_list = list()
    y_list = list()
    ...
    clothoid2 = SolveG2(s_x, s_y, s_theta, s_curvature,
                        e_x, e_y, e_theta, e_curvature)

    for point in clothoid2:
        clothoid_list.append(point)
        clothoid_length_list.append(point.length)
        total_length += point.length

    traj_num = int(np.ceil(total_length/resolution + 0.5))
    ...
    discretize_pts["x_list"] = x_list
    discretize_pts["y_list"] = y_list
    discretize_pts["theta_list"] = theta_list
    discretize_pts["curvature_list"] = curvature_list
    discretize_pts["s_list"] = s_list
    discretize_pts["total_length"] = total_length
    return discretize_pts


class GenReflineByClothoid():  # 两辆车
    def __init__(self, car_start_x, car_start_y, car_start_heading, car_start_curvature, car_end_x, car_end_y, car_end_heading, car_end_curvature, car2_start_x, car2_start_y, car2_start_heading, car2_start_curvature, car2_end_x, car2_end_y, car2_end_heading, car2_end_curvature,
        ...
        self.car2_start_x = car2_start_x
        self.car2_start_y = car2_start_y
        self.car2_start_heading = car2_start_heading
        self.car2_start_curvature = car2_start_curvature

        self.car2_end_x = car2_end_x
        self.car2_end_y = car2_end_y
        self.car2_end_heading = car2_end_heading
        self.car2_end_curvature = car2_end_curvature

        self.car2_length = 4.0
        self.car2_width = 2.0
    ...
    def gen_car2_refline(self) -> dict:
        car_refline = dict()
        ...
        clothoid2_car2 = SolveG2(self.car2_start_x, self.car2_start_y, self.car2_start_heading, self.car2_start_curvature,
                                 self.car2_end_x, self.car2_end_y, self.car2_end_heading, self.car2_end_curvature)

        for point in clothoid2_car2:
            clothoid_list.append(point)
            clothoid_length_list.append(point.length)
            total_length += point.length
...
def CalPredictPath(car: Vehicle, car2: Vehicle, tar1_end_pt1: tuple, tar1_end_pt2: tuple, tar2_end_pt1: tuple, tar2_end_pt2: tuple):
    car_path_list = list()
    car2_path_list = list()

    s_car_x = car.state.x
    s_car_y = car.state.y
    e1_car_x = tar1_end_pt1[0]
    e1_car_y = tar1_end_pt1[1]
    e2_car_x = tar1_end_pt2[0]
    e2_car_y = tar1_end_pt2[1]

    s_car2_x = car2.state.x
    s_car2_y = car2.state.y
    e1_car2_x = tar2_end_pt1[0]
    e1_car2_y = tar2_end_pt1[1]
    e2_car2_x = tar2_end_pt2[0]
    e2_car2_y = tar2_end_pt2[1]

    if s_car_x < 20.0:
        car_pts1 = list()
        car_pts2 = list()
        clothoid2_car11 = SolveG2(s_car_x, s_car_y, car.state.theta, car.state.curvature,
                                  e1_car_x, e1_car_y, math.radians(90), 0)
        clothoid2_car12 = SolveG2(s_car_x, s_car_y, car.state.theta, car.state.curvature,
                                  e2_car_x, e2_car_y, math.radians(90), 0)
        for i in range(len(["x_list"])):
            car_pt = dict()
            car_pt["x"] = pt[]

    if s_car2_y < 20.0:
        clothoid2_car21 = SolveG2(s_car2_x, s_car2_y, car.state.theta, car.state.curvature,
                                  e1_car2_x, e1_car2_y, math.radians(0), 0)
        clothoid2_car22 = SolveG2(s_car2_x, s_car2_y, car2.state.theta, car2.state.curvature,
                                  e2_car2_x, e2_car2_y, math.radians(0), 0)


def PlotRefline(ego, car, car2, ego_refline, car_refline, car2_refline, map_element_pts, sample_ego_pts_list, sample_car_pts_list, best_policy, outpath):
    ...
    car2_x_list = car2_refline["x_list"]
    car2_y_list = car2_refline["y_list"]
    ...
    agent2_vertice = list(GetVertice(car2.id, car2.param, car2.state, 0, 0))
    ...
    car2_shape_xs = [agent2_vertice[0][0], agent2_vertice[1][0],
                     agent2_vertice[2][0], agent2_vertice[3][0], agent2_vertice[0][0]]
    car2_shape_ys = [agent2_vertice[0][1], agent2_vertice[1][1],
                     agent2_vertice[2][1], agent2_vertice[3][1], agent2_vertice[0][1]]

    plt.plot(ego_shape_xs, ego_shape_ys)
    plt.plot(car_shape_xs, car_shape_ys)
    plt.plot(car2_shape_xs, car2_shape_ys)
    ...
    plt.plot(car2_y_list, car2_x_list, '--')
...
def difference_function(pt1, pt2, num):
    discrete_val = (pt1[0], pt1[0])
    diff_x = (pt2[0] - pt1[0]) / num
    diff_y = (pt2[1] - pt2[1]) / num
    for i in range(num):
        discrete_val.append(pt1[0] + (i + 1) * diff_x,
                            pt1[1] + (i + 1) * diff_y)
    return discrete_val


def GenReflinePts(ego_refline, car_refline, car2_refline, ref_pairs, ref_pairs2):
    car_reflines_pts = list()
    car2_reflines_pts = list()

    ego_refline_pts = [(x, y) for y, x in zip(
        ego_refline["x_list"], ego_refline["y_list"])]

    for pair in ref_pairs:
        for i in range(len(pair['cons_starts'])):
            car_refline_pts = difference_function(pt1, pt2, num)

        car_refline_pts = []

    car_refline_pts = []
    for i in range(240):
        car_refline_pts.append((-5, 80 - i*0.25))
    ...
    car2_refline_pts = []
    for i in range(240):
        car2_refline_pts.append((5, 80 - i*0.25))
    for i in range(len(car2_refline["x_list"])):
        car2_refline_pts.append(
            (car2_refline["y_list"][i], car2_refline["x_list"][i]))
    for i in range(240):
        car2_refline_pts.append((20 + i*0.25, 5))
    return ego_refline_pts, car_refline_pts, car2_refline_pts
...
def combine_refline(reflines, cons_starts, cons_ends) -> list:
    ref_pairs = list()
    for i in range(len(reflines)):
        ref_pair = dict()
        ref_pair['ref'] = reflines[i]
        ref_pair['cons_starts'] = cons_starts[i]
        ref_pair['cons_ends'] = cons_ends[i]
...
if __name__ == '__main__':
    refline_generator = GenReflineByClothoid(20, -5, math.radians(180), 0, -5, 20, math.radians(
        90), 0,
        20, 5, math.radians(180), 0, 5, 20, math.radians(
        90), 0, -80, 5, math.radians(0), 0, 80, 5, math.radians(0), 0)
    refline2_generator = GenReflineByClothoid(20, -15, math.radians(180), 0, -5, 20, math.radians(
        90), 0,
        20, 15, math.radians(180), 0, 5, 20, math.radians(
        90), 0, -80, 5, math.radians(0), 0, 80, 5, math.radians(0), 0)
    ego_refline = refline_generator.gen_ego_refline()
    car_refline = refline_generator.gen_car_refline()
    car2_refline = refline_generator.gen_car2_refline()
    car_refline2 = refline2_generator.gen_car_refline()
    car2_refline2 = refline2_generator.gen_car2_refline()

    ref_pairs = combine_refline([car_refline, car_refline2], [
                                [(-5, 20), (20, -5)], [(-5, 20), (20, -15)]], [[(-5, 80), (80, -5)], [(-5, 80), (80, -15)]])
    ref_pairs2 = combine_refline([car2_refline, car2_refline2], [
        [(5, 20), (20, 5)], [(15, 20), (20, 5)]], [[(5, 80), (80, 5)], [(15, 80), (80, 5)]])

    map_element_pts = [[[0, 0], [80, 20]], [[-10, -10], [80, 20]], [[10, 10], [80, 20]], [[-20, -20], [80, 20]], [[20, 20], [80, 20]],
                       [[0, 0], [-80, -20]], [[-10, -10], [-80, -20]], [[10, 10],
                                                                        [-80, -20]], [[-20, -20], [-80, -20]], [[20, 20], [-80, -20]],
                       [[-80, -20], [0, 0]], [[-80, -20], [10, 10]], [[-80, -20],
                                                                      [20, 20]], [[-80, -20], [-10, -10]], [[-80, -20], [-20, -20]],
                       [[80, 20], [0, 0]], [[80, 20], [10, 10]], [[80, 20], [20, 20]], [[80, 20], [-10, -10]], [[80, 20], [-20, -20]]]
    ego_refline_pts, car_refline_pts, car2_refline_pts = GenReflinePts(
        ego_refline, car_refline, car2_refline)

    param = Param()
    ego_state = State(5, -70, 5, math.radians(90), 0, 0)
    other_state = State(-5, 70, 5, math.radians(-90), 0, 0)
    other_state2 = State(50, 5, 5, math.radians(180), 0, 0)
    ego = Vehicle(1, ego_refline_pts, ego_state, param)
    other = Vehicle(2, car_refline_pts, other_state, param)
    other2 = Vehicle(3, car2_refline_pts, other_state2, param)
    ...
    while(ego.state.y < 20 and out_index < 20):
        ...
        PlotRefline(ego, other, other2, ego_refline, car_refline, car2_refline, map_element_pts,
